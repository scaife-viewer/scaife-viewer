      ...
      <xsl:choose>
        <xsl:when test="py:token_type(.) = 'w'">
          <xsl:element name="t">
            ...
            <xsl:attribute name="passage-idx"><xsl:value-of select="py:token_passage_idx(.)" /></xsl:attribute>
            <xsl:attribute name="ve-ref"><xsl:value-of select="py:token_ve_ref(.)" /></xsl:attribute>
            <xsl:value-of select="." />
          </xsl:element>
        </xsl:when>
